...
/**
************************************************************************
* @brief:      	BMI088_read(BMI088_DATA* imudata)
* @param:       imudata - 传感器数据结构体
* @retval:     	void
* @details:    	读取BMI088传感器数据，包括加速度、陀螺仪和温度
************************************************************************
**/
void BMI088_read_(BMI088_DATA* imudata)
{
    uint8_t buf[8] = {0, 0, 0, 0, 0, 0};
    int16_t bmi088_raw_temp;
    ...
    BMI088_accel_read_muli_reg(BMI088_ACCEL_XOUT_L, buf, 6);
    bmi088_raw_temp = (int16_t)((buf[1]) << 8) | buf[0];
    imudata->ax = bmi088_raw_temp * BMI088_ACCEL_SEN;
    bmi088_raw_temp = (int16_t)((buf[3]) << 8) | buf[2];
    imudata->ay = bmi088_raw_temp * BMI088_ACCEL_SEN;
    bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
    imudata->az = bmi088_raw_temp * BMI088_ACCEL_SEN;
    ...
    {
        bmi088_raw_temp = (int16_t)((buf[3]) << 8) | buf[2];
        imudata->gx = bmi088_raw_temp * BMI088_GYRO_SEN;
        bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
        imudata->gy = bmi088_raw_temp * BMI088_GYRO_SEN;
        bmi088_raw_temp = (int16_t)((buf[7]) << 8) | buf[6];
        imudata->gz = bmi088_raw_temp * BMI088_GYRO_SEN;
    }
    ...
    imudata->temp = bmi088_raw_temp * BMI088_TEMP_FACTOR + BMI088_TEMP_OFFSET;

    // printf("%f,%f,%f\n", imudata->gx, imudata->gy, imudata->gz);
}
/**
************************************************************************
* @brief:      	BMI088_read_gyro(IMU_DATA* imudata)
* @param:       imudata - 传感器数据结构体
* @retval:     	void
* @details:    	读取BMI088陀螺仪数据
************************************************************************
**/
void BMI088_read_gyro(BMI088_DATA* imudata)
{
    uint8_t buf[8] = {0, 0, 0, 0, 0, 0};
    int16_t bmi088_raw_temp;
...
************************************************************************
* @brief:      	BMI088_read_gyro(IMU_DATA* imudata)
* @param:       imudata - 传感器数据数组
* @retval:     	void
* @details:    	读取BMI088温度数据
************************************************************************
**/
void BMI088_read_temperate(BMI088_DATA* imudata)
{
    uint8_t buf[8] = {0, 0, 0, 0, 0, 0};
    int16_t bmi088_raw_temp;
